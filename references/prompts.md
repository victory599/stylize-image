...
生成前读本文件。占位符逐项替换，模板中不得残留花括号。长英文段以各表为准，本文件只写**装配规则**。
...
下列之一：`1:1 aspect ratio` / `4:3 aspect ratio` / `3:4 aspect ratio` / `16:9 aspect ratio` / `9:16 aspect ratio`。
...
|------|------|
| 氛围=无 | 空（删空行） |
| 表内氛围 | 按当前忠实度取 [mood-modifiers.md](mood-modifiers.md) 对应英文列整段：形似/再演绎→「完整版」；紧贴→「紧贴短版」 |
...
仅有标题区模板使用。取 [banner-layouts.md](banner-layouts.md) 对应行（勿改写）。布局=无时不用。
...
选模板：布局=无 →「无标题区」；布局∈{底,顶,侧} →「有横幅」；布局=溶字 →「溶字」。
